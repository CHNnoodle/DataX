...
public class HdfsHelper {
    ...
    public void textFileStartWrite(RecordReceiver lineReceiver, Configuration config, String fileName,
        ...
        //
        char fieldQuote = config.getChar(Key.FIELD_QUOTE);
        char fieldEscape = config.getChar(Key.FIELD_ESCAPE);
        ...
        try {
            ...
            while ((record = lineReceiver.getFromReader()) != null) {
                MutablePair<Text, Boolean> transportResult = transportOneRecord(record, fieldDelimiter, fieldQuote, fieldEscape, columns, taskPluginCollector);
//                MutablePair<Text, Boolean> transportResult = transportOneRecord(record, fieldDelimiter, columns, taskPluginCollector);
    ...
    public static MutablePair<Text, Boolean> transportOneRecord(
            Record record, char fieldDelimiter, char fieldQuote, char fieldEscape, List<Configuration> columnsConfiguration, TaskPluginCollector taskPluginCollector) throws IOException {
        ...
        StringWriter writer;
        CSVWriter csv;
        if (null != transportResultList && null != transportResultList.getLeft()) {

            //
            writer = new StringWriter();
            csv = new CSVWriter(writer, fieldDelimiter, fieldQuote, fieldEscape, "");
            csv.writeNext(transportResultList.getLeft().stream().map(e -> String.valueOf(e)).collect(Collectors.toList()).toArray(new String[0]));
            csv.close();
            Text recordResult = new Text(writer.toString());
